%% ElemStrain function
function [strain_matrix, stress_matrix] = ElemStrain(iel,ndime,mate,coor,conn,u)
    % 三角形三點座標
    ...
    u_iel(2) = u((conn(1,iel)-1)*ndime+2);
    u_iel(3) = u((conn(2,iel)-1)*ndime+1);
    u_iel(4) = u((conn(2,iel)-1)*ndime+2);
    u_iel(5) = u((conn(3,iel)-1)*ndime+1);
    u_iel(6) = u((conn(3,iel)-1)*ndime+2);
