...
for i = nstpho+1
    ...
    while(erri > tol && niti < maxit) % Check steady state
        niti = niti+1;
        kglob = GlobStif_BBar(ndime,nnode,nelem,nelnd,mate,coor,conn,wglob); % hypo
...
%     if(i == nstpho+1)
    if(iprt == nprtho || iprt == 0)
        ...
        stress_e_hypo(indi) = stress_matrix(500,3);
        strain_e_hypo(indi) = strain_matrix(500,3);
...
stress_e_hypo = stress_e_hypo*(-2.4*10e-2);
strain_e_hypo = strain_e_hypo*(-4.2*10e-11);
