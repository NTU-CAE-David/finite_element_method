...
%          Demonstrates Bbar method for avoiding volumetric locking
%
...
%
      for a=1:nelnodes
          for i = 1:ndof
              Bvol(a,i) = Bvol(a,i) + dNdx(a,i)*w(intpt)*dt;
          end
      end
   ...
   for a=1:nelnodes
       for i=1:ndof
         Bvol(a,i) = Bvol(a,i)/(ncoord*el_vol);
...
%
      ...
      strain(1,1) = strain(1,1) - ekk/ncoord + omega;
      strain(2,2) = strain(2,2) - ekk/ncoord + omega;
      if (ncoord==3)
          strain(3,3) = strain(3,3) - ekk/ncoord + omega;
      end
%
              ...
              for j = 1:ncoord
                for l = 1:ncoord
                  ...
                  kel(col,row) = kel(col,row) + ...
                           dsde(j,j,k,l)*dNdx(b,l)*(Bvol(a,i) - dNdx(a,i)/ncoord)*w(intpt)*dt;
                  kel(col,row) = kel(col,row) + ...
                           dsde(i,j,l,l)*dNdx(a,j)*(Bvol(b,k) - dNdx(b,k)/ncoord)*w(intpt)*dt;
                  kel(col,row) = kel(col,row) + ...
                           dsde(j,j,l,l)*(Bvol(b,k)-dNdx(b,k)/ncoord)*(Bvol(a,i) - dNdx(a,i)/ncoord)*w(intpt)*dt;              
                end
              end
            end
          end
